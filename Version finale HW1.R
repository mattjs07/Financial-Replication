...
ID <- table(df$PERMNO) %>%  as.matrix() %>% rownames() %>% as.integer %>%  as.list()  # All the PERMNOs
...
DATES <- table(data$date)  %>%  as.matrix() %>% rownames() %>% as.integer %>%  as.list() #list with all DATES, needed for following functions
